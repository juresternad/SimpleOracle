...
struct Round {
    ...
    uint256 allWeight;
    uint256[] indexedPrices;
    uint256[] indexedWeights;
    uint256[] index;
...
function revealVote(uint256 _vote, uint256 _salt, uint256 roundNumber) public {
    ...
    rounds[roundNumber].allWeight += weights[msg.sender];
    rounds[roundNumber].votes[msg.sender].vote = _vote;
    rounds[roundNumber].indexedPrices.push(_vote);
    rounds[roundNumber].indexedWeights.push(weights[msg.sender]);
    uint256 i = rounds[roundNumber].indexedPrices.length;
    rounds[roundNumber].index.push(i);
}
...
function addOrUpdate (address voter, uint256 weight) public onlyOwner {
    if(weight == 0){
        weights[voter] = 0;
